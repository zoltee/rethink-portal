class BCUser{
	...
	isUserLoggedIn() {
		...
		if (userData &&  profileId) {
			if (!this.brainCloudClient.brainCloudClient.isAuthenticated()){
				this.brainCloudClient.reconnect(); //async
			}else{
				return true;
			}
		}
	...
	loginUser(email, password, create = false) {
		return new Promise((resolve, reject) => {
	...
	updateUsername(username){
		return this.updateAttributes({username});
	}
